  ...
  return (
    <div id="property-search-filter-hub" className="w-full max-w-3xl mx-auto space-y-3 text-left">
      ...
      <div className="w-full relative z-10">
        ...
        {/* Live Result Counter & Quick Summary */}
        <div className="flex items-center justify-between px-3 pt-2 text-xs text-slate-600 dark:text-slate-400 font-medium">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span>
              Showing <strong className="text-sky-950 dark:text-sky-200 font-bold">{totalResults}</strong> properties
            </span>
            {filters.city !== 'All Locations' && (
              <span className="bg-sky-100 dark:bg-sky-950 text-sky-800 dark:text-sky-300 px-2 py-0.5 rounded-full text-[11px] font-semibold border border-sky-200 dark:border-sky-800">
                in {filters.city}
              </span>
            )}
            {filters.category !== 'All Types' && (
              <span className="bg-blue-100 dark:bg-slate-800 text-blue-800 dark:text-blue-300 px-2 py-0.5 rounded-full text-[11px] font-semibold border border-blue-200 dark:border-slate-700">
                {filters.category}
              </span>
            )}
          </div>
          {isFiltered && (
            <button
              type="button"
              onClick={onResetFilters}
              className="text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-200 font-semibold underline cursor-pointer text-[11px]"
            >
              Clear All
            </button>
          )}
