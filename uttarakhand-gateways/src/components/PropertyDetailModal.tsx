        ...
        <div className="overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 space-y-4">
              ...
              <div className="bg-gradient-to-b from-sky-50/90 to-blue-50/60 dark:from-slate-800/90 dark:to-slate-800/60 p-5 rounded-3xl border border-sky-200 dark:border-slate-700 shadow-md space-y-3.5">
                
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-[#0c2340] dark:text-white flex items-center gap-1.5">
                    <User className="w-4 h-4 text-sky-600" />
                    <span>{property.sellerName || 'Direct Property Owner'}</span>
                  </h3>
                ...
                <div className="p-3 rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-sky-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-300 flex items-start gap-2">
                  <Lock className="w-4 h-4 text-sky-600 dark:text-sky-400 flex-shrink-0 mt-0.5" />
                  <div>
                    <span className="font-semibold text-slate-900 dark:text-white">Phone Privacy Protected: </span>
                    <span>To prevent unsolicited spam, the owner's phone number is masked. You can request access or start a direct chat.</span>
                  </div>
                </div>
                ...
                {phoneRequestStatus === 'approved' ? (
                  <div className="space-y-2 p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-700">
                    <div className="flex items-center justify-between text-xs font-bold text-emerald-800 dark:text-emerald-300">
                      <span className="flex items-center gap-1">
                        ...
                        <span>Owner Approved Contact:</span>
                      </span>
                      <span className="font-mono text-sm">{approvedPhoneNumber || property.sellerPhone || '+91 7535981704'}</span>
                    </div>

                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <a
                        href={`tel:${(approvedPhoneNumber || property.sellerPhone || '+917535981704').replace(/[^0-9+]/g, '')}`}
                      ...
                      <a
                        href={`https://wa.me/${(approvedPhoneNumber || property.sellerPhone || '917535981704').replace(/[^0-9]/g, '')}`}
                        target="_blank"
                  ...
                  <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-700 text-xs text-amber-800 dark:text-amber-300 flex items-center justify-between">
                    <span className="font-semibold flex items-center gap-1.5">
                      ...
                      <span>Phone Request Pending Approval</span>
                    </span>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-4 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs transition-all flex items-center justify-center gap-2 shadow-xs cursor-pointer"
                  >
                    <Lock className="w-4 h-4" />
                    <span>Request Owner's Phone Number (+91 98••••••••)</span>
                  </button>
                )}

                {/* Direct Chat with Owner Button */}
                {onStartChat && (
                  <button
                    onClick={() => {
                    ...
                    className="w-full py-2.5 px-4 rounded-xl bg-white dark:bg-slate-700 hover:bg-sky-50 dark:hover:bg-slate-600 text-sky-800 dark:text-sky-300 font-bold text-xs border border-sky-200 dark:border-slate-600 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xs"
                  >
                    <MessageSquare className="w-4 h-4 text-sky-600 dark:text-sky-400" />
                    <span>Chat with Owner in Inbox</span>
                  </button>
                )}
                ...
                <button
                  onClick={() => onToggleFavorite(property.id)}
                  className="w-full py-2 px-4 rounded-xl bg-white dark:bg-slate-700 hover:bg-sky-50 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-xs font-medium border border-sky-200 dark:border-slate-600 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-xs"
                >
